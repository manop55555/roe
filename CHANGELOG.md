...
- **Color control** with `--color=<auto|always|never>`, auto-gated to a TTY by
  default: output is colorized only when stdout is a terminal. `--no-color` is an
  alias for `--color=never`; `NO_COLOR` forces it off; `--color=always` keeps
  color when piped (e.g. `| less -R`).
- **Usage error on no arguments**: bare `roe` prints help to stderr and exits 1,
  consistent with other usage errors; `--help`/`-h` print to stdout and exit 0.
- **`--bytes <n>` clamping** for both a `--hex --addr` window and a named-section
  dump, printing a `note:` to stderr when the count is clamped to the
  section/range boundary.
- **Invalid-hex handling for `--raw-bytes`**: prints a `note:` to stderr when
  text input is not valid hex and is read as raw bytes (`--hex-input` forces
  hex); `--hex-input` reports `input contained no valid hex digits` distinctly
  from an empty stdin.
