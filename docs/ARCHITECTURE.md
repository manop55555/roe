...
- `binary::load_file`: reads the first bytes, dispatches to the parser agent, and preserves the first 16 bytes for diagnostics.
- Unknown input returns `ErrorCode::UnsupportedFormat` with a first-16-byte hex dump generated by `binary::first_bytes_hex`.
...
- `fuzz/elf_parser_fuzzer.cpp` owned by `agent-security` with ELF parser support from `agent-elf`.
- `fuzz/macho_parser_fuzzer.cpp` owned by `agent-security` with Mach-O parser support from `agent-macho`.
- `fuzz/pe_parser_fuzzer.cpp` owned by `agent-security` with PE parser support from `agent-pe`.
- `fuzz/archive_parser_fuzzer.cpp` owned by `agent-security` with archive parser support from `agent-archive`.

Fuzzers parse from owned byte vectors, exercise metadata accessors and adapter
creation, and must never shell out or depend on host toolchains.

## Directory Ownership

- `docs/`, `CMakeLists.txt`, `include/roe/`: architecture agent.
- `src/elf/`, `include/roe/elf.hpp`: ELF parser agent.
- `src/macho/`, `include/roe/macho.hpp`: Mach-O parser agent.
- `src/pe/`, `include/roe/pe.hpp`: PE/COFF parser agent.
- `src/archive/`, `include/roe/archive.hpp`: archive parser agent.
- `src/disasm/`, `src/disasm/arch/`, `include/roe/disasm.hpp`: disassembly and multi-arch agents.
- `src/resolver/`, `include/roe/resolver.hpp`: resolver agent.
- `src/debug/`, `include/roe/debug.hpp`: debug/source agent.
- `src/features/`, `src/watcher/`, `include/roe/features.hpp`, `include/roe/watcher.hpp`: feature agent.
- `src/format/`, `include/roe/format.hpp`: formatting agent.
- `src/cli/`, `src/main.cpp`, `include/roe/cli.hpp`: CLI and help agents.
- `tests/`, `scripts/test-arch.sh`, `scripts/check-readme.sh`, `scripts/coverage.sh`: test agent.
- `fuzz/`, `scripts/fuzz.sh`, `SECURITY.md`: security agent.
- `packaging/`, `install.sh`, `Dockerfile`, `.github/workflows/`: packaging agent.
- `LICENSE`, `NOTICE`, `CONTRIBUTING.md`, `docs/LICENSING.md`, SPDX headers: legal agent.
...
- Parser-specific models remain available so parser agents can validate deeply without losing format detail.
