...
- Ship a focused v1.0.0: ELF is the only parsed format; Mach-O, PE/COFF, and archives are detected by magic and reported as unsupported with a clear message, because an honest, fully-tested ELF release beats four half-parsers. The `BinaryFile` interface keeps the door open.
...
- Select ARM vs Thumb with `--arch thumb` instead of auto-detecting `$a/$t` mapping symbols, keeping the disassembler stateless; mapping-symbol-driven mode switching is a future enhancement.
- Leave the GitHub owner as the literal `USER` placeholder in install/Docker/package URLs; it is substituted at release time by the repository owner.
